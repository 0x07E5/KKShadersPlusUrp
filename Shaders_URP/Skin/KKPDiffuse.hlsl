// KKShadersPlus URP - Skin Diffuse
// URP Version

#ifndef KKP_DIFFUSE_URP
#define KKP_DIFFUSE_URP
...
float3 GetDiffuse(Varyings i) {
    ...
    overTex1.rgb = mask * overTex1.rgb + overTex1Col.rgb;
    ...
    // Color MainTex
    float4 col0 = max(_Col0, 1E-06);
    float4 col1 = max(_Col1, 1E-06);
    float4 col2 = max(_Col2, 1E-06);
    float4 col3 = max(_Col3, 1E-06);
    
    float2 colorUV = i.uv0 * _ColMask_ST.xy + _ColMask_ST.zw;
    float4 colorMask = SAMPLE_TEX2D_SAMPLER(_ColMask, _MainTex, colorUV);
    
    float3 color = col0.rgb;
    color = colorMask.r * (col1.rgb - color) + color;
    color = colorMask.g * (col2.rgb - color) + color;
    color = colorMask.b * (col3.rgb - color) + color;
