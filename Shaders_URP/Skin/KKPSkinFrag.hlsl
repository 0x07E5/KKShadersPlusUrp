// KKShadersPlus URP - Skin Fragment
// URP Version

#ifndef KKP_SKINFRAG_URP
#define KKP_SKINFRAG_URP
...
half4 frag(Varyings i, half frontFace : VFACE) : SV_Target
{
    ...
    // Diffuse and color maps KK uses for shading
    ...
    normal = NormalAdjust(i, finalCombinedNormal, faceDir);
    ...
    float3 bodyFresCol = applySaturation(bodyFres * _KKPRimColor.rgb + (1 - bodyFres) * diffuse, _Saturation);
    ...
    float shadowAttenuation = GetShadowAttenuation(i, vertexLighting.a, normal, worldLightDir, viewDir, shadowCoord);
    ...
    float specularMesh = GetMeshSpecular(i, vertexLights, specularNormal, viewDir, worldLightDir, mainLightColor, specularColorMesh);
    ...
    float3 cumCol = (GetLambert(worldLightDir, normal) + vertexLighting.a + 0.5) * float3(0.149999976, 0.199999988, 0.300000012) + float3(.850000024, 0.800000012, 0.699999988);
    ...
    bodyShine = (mainLightColor + vertexLighting.rgb * vertexLightRamp) * float3(0.600000024, 0.600000024, 0.600000024) + _CustomAmbient.rgb;
    ...
    return half4(max(finalCol, 1E-06), 1); 
}
