// KKShadersPlus URP - Eye Plus Fragment
// URP Version

#ifndef KKP_EYEPLUSFRAG_URP
#define KKP_EYEPLUSFRAG_URP
...
half4 frag(Varyings i) : SV_Target {
    ...
    float2 expressionUV = float2(dot(i.tanWS.xyz, viewDir),
    ...
    float4 overTex1 = SAMPLE_TEX2D(_overtex1, i.uv1 * _overtex1_ST.xy + _overtex1_ST.zw);
    overTex1 = overTex1.a * _overcolor1.rgba;
    float4 overTex2 = SAMPLE_TEX2D(_overtex2, i.uv2 * _overtex2_ST.xy + _overtex2_ST.zw);
...
#endif
    float lambert = max(dot(worldLightDir, i.normalWS.xyz), 0.0) + vertexLighting.a;
    ...
    emissionUV = emissionUV + float2(dot(i.tanWS.xyz, viewDir), dot(i.bitanWS, viewDir)) * -0.06 * _ExpressionDepth;
