// KKShadersPlus URP - Hair Vert Frag
// URP Version

#ifndef KKP_HAIRVF_URP
#define KKP_HAIRVF_URP
...
half4 frag(Varyings i, half frontFace : VFACE) : SV_Target
{
    ...
    float3 adjustedNormal = NormalAdjust(i, GetNormal(i), faceDir);
    ...
    float3 specularLightColor = _UseLightColorSpecular ? mainLightColor * _SpecularColor.a : _SpecularColor.rgb * _SpecularColor.a;
...
#endif
    // For some reason hair gloss is messed up on some hairs unless you invert
    ...
    float hairGloss = (hairGloss1.r - hairGloss2.r) * 0.5f;
    ...
    float3 shadowCol = _ShadowColor.rgb + 1E-06 - 0.5;
    ...
    finalAmbientShadow = saturate(finalAmbientShadow);
    float minusAmbientShadow = finalAmbientShadow.r - 1;
    ...
    shading = (mainLightColor + vertexLighting.rgb * vertexLightRamp) * float3(0.600000024, 0.600000024, 0.600000024) + _CustomAmbient.rgb;
