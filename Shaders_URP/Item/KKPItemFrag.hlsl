// KKShadersPlus URP - Item Fragment
// URP Version

#ifndef KKP_ITEM_FRAG_URP
#define KKP_ITEM_FRAG_URP
...
half4 frag(Varyings i, half faceDir : VFACE) : SV_Target {
    // Get main light
    ...
    float3 viewDir = normalize(GetCameraPositionWS() - i.posWS.xyz);
    ...
    float3 kkpFresCol = applySaturation(kkpFres * _KKPRimColor.rgb + (1 - kkpFres) * diffuse, _Saturation);
    ...
    for(int j = 0; j < 4; j++) {
        ...
        float3 halfVector = normalize(viewDir + light.dir) * saturate(MaxGrayscale(light.col.rgb));
    ...
    float anotherRamp = SAMPLE_TEX2D(_AnotherRamp, anotherRampUV).x;
    ...
    detailSpecularOffset.x = dot(i.tanWS.xyz, viewDir);
    ...
    specular = SAMPLE_TEX2D(_RampG, lightRampUV).x * _UseRampForSpecular + specular * (1 - _UseRampForSpecular);
#endif
    ...
    finalDiffuse = GetBlendReflections(i.uv0, max(finalDiffuse, 1E-06), normal, viewDir, kkMetalMap, finalRamp);
    ...
    return half4(max(finalDiffuse, 1E-06), alpha);
}
